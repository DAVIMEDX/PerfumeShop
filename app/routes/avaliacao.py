...
@router.get("/avaliacoes/perfume/{perfume_id}", response_model=List[Avaliacao])
def listar_avaliacoes_perfume(perfume_id: int, db: Session = Depends(database.get_db)):
    return crud.listar_avaliacoes_por_perfume(db, perfume_id)
...
@router.get("/avaliacoes/perfume/{perfume_id}/media")
def media_avaliacao(perfume_id: int, db: Session = Depends(database.get_db)):
    return {"media": crud.media_avaliacao_por_perfume(db, perfume_id)}
